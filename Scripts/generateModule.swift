...
func registerModulePaths() {
    updateFileContent(
        ...
        finding: "enum \(layer.rawValue): String, ModularPathConvertable {\n",
...
func makeScaffold(target: MoudlarTargetType) {
    _ = try? bash.run(
        commandName: "tuist",
...
print("모듈 이름: \(moduleName)")
print("interface: \(hasInterface), testing: \(hasTesting), unitTests: \(hasUnitTests), demo: \(hasDemo)")
print("------------------------------------------------------------------------------------------------------------------------")
...
struct Bash: CommandExecuting {
    ...
    private func resolve(_ command: String) throws -> String {
        ...
        // /bin/bash -l -c "which <command>" , 로그인 쉘로 which command 실행
